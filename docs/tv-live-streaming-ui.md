...
Bilibili's live formats (`tool/native-dash-smoke/`, 2026-07-20) settled the approach:
...
So live routes the **CDN m3u8 straight to the adaptive (PlusPlayer) engine**
(`VideoFormat.hls`, `PlayerEngine.adaptiveStreaming`) — **no byte-pump, no `/direct`
relay, no binary patch**. Two Dart changes deliver it:
...
2. `avplay_media_player.open()` — a live `.m3u8` `videoUri` (only live yields one; VOD is
   dual-stream DASH or a durl MP4) routes to the adaptive engine with the **direct CDN
   url**, alongside the existing native-DASH and `/direct` byte-pump paths.

The plugin `play()` guard (skip replay-seek when `duration.end == 0`) is required so the
non-seekable live stream actually starts.

...
/// Large 16:9 video card for the TV grid.
///
/// Layout: cover with a bottom gradient scrim and duration badge, then a
/// two-line title and an uploader + view/danmaku meta row. Sized by the
/// parent grid; the text block is [TvTheme.cardInfoHeight] tall.
class TvVideoCard extends StatelessWidget {
  ...
  Widget _buildMetaRow() {
    final stats = <Widget>[
      if (item.stat.view case final view?)
        _buildStat(Icons.play_arrow_rounded, NumUtils.numFormat(view)),
      if (item.stat.danmu case final danmu?)
        _buildStat(Icons.subtitles_outlined, NumUtils.numFormat(danmu)),
    ];
